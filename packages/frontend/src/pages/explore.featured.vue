	...
	<div v-else-if="tab === 'polls'">
		<template v-if="ltlAvailable || gtlAvailable">
			<MkFoldableSection v-if="ltlAvailable" class="_margin">
				...
				<MkNotesTimeline :pagination="paginationForPollsLocal" :disableAutoLoad="true"/>
			</MkFoldableSection>
			...
			<MkFoldableSection v-if="gtlAvailable" class="_margin">
				...
				<MkNotesTimeline :pagination="paginationForPollsRemote" :disableAutoLoad="true"/>
			</MkFoldableSection>

			<MkFoldableSection v-if="gtlAvailable" class="_margin">
				<template #header><i class="ph-timer ph-bold ph-lg" style="margin-right: 0.5em;"></i>{{ i18n.ts.pollsExpired }}</template>
				<MkNotesTimeline :pagination="paginationForPollsExpired" :disableAutoLoad="true"/>
			</MkFoldableSection>
		</template>
		<template v-else>
